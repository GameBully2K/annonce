	...
	<div class="animate-slideUp flex flex-col card m-10 p-5 md:w-[40%]" >
		<div class="flex justify-center items-center p-2 ">
			<a href="/login/google"
				...
				<span class=" text-md md:text-lg font-semibold ml-4">Se connecter avec Google</span>
			</a>
		</div>
		...
		{#if codeSent}
			<div class="card flex flex-col p-4 gap-y-5 justify-center items-center text-center">
				<h1 class="h1 font-semibold ">Vérifier le Code</h1>
				<p class="text-s">pour : {email}</p>
				<form method="post" class="flex flex-col gap-y-2" use:enhance>
					...
					<button type="submit" class="variant-filled btn" >Continuer</button>
					<button type="button" class="variant-outlined btn" on:click={()=> codeSent = false } >Corriger l'email</button>
					<p class="text-khoukhi">{form?.message ?? ''}</p>
				</form>
			</div>
		{:else}
			<div class="card flex flex-col p-4 h-[50%] gap-y-5 justify-center items-center text-center">
				<h1 class="h1 font-semibold ">Se connecter</h1>
